...
buildGraph params root =
  let (nextNode, base) = execRWS (buildRoot root) (newCFContext params) 0
    ...
    build' t = case t of
      ...
      TA_Assignment id op var@(TA_Variable _ name indices) rhs -> do
        ...
        read <-
          if op == "="
            then none
            else -- This is += or something
              newNodeRange $ applySingle $ IdTagged id $ CFReadVariable name

        write <-
          newNodeRange $
            applySingle $
              IdTagged id $
                CFWriteVariable name $
                  if null indices
                    then CFValueInteger
                    else CFValueArray
  ...
  where
    ...
    handleDeclare (cmd NE.:| args) = do
      ...
      where
        ...
        unsetOptions :: String
        unsetOptions =
          let strings = mapMaybe getLiteralString args
              plusses = filter ("+" `isPrefixOf`) strings
           in concatMap (drop 1) plusses
    ...
    handleRead (cmd NE.:| args) = newNodeRange $ CFApplyEffects main
      where
        ...
        fallback =
          let names = reverse $ map fromJust $ takeWhile isJust $ map (\c -> sequence (getId c, getLiteralString c)) $ reverse args
           ...
           in map (\(id, name) -> IdTagged id $ CFWriteVariable name value) namesOrDefault
...
inlineSubshells graph = relinkedGraph
  where
    ...
    relink graph (node, label, start, end, incoming, outgoing) =
      let -- Link CFExecuteSubshell to the CFEntryPoint
       ...
       in subshellToStart `safeUpdate` (endToNexts `safeUpdate` graph)
