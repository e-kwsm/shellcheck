...
sliceFile fix lines =
  ...
  where
    (minLine, maxLine) =
        foldl
            (\(mm, mx) pos -> ((min mm $ fromIntegral $ posLine pos), (max mx $ fromIntegral $ posLine pos)))
            (maxBound, minBound)
            $ concatMap (\x -> [repStartPos x, repEndPos x])
            $ fixReplacements fix
...
showFixedString color comments lineNum fileLines =
    let line = fileLines ! fromIntegral lineNum
     in case mapMaybe pcFix comments of
            [] -> return ()
            fixes -> do
  ...
  where
    ...
    makeArrow =
        let sameLine = lineNo comment == endLineNo comment
         ...
         in if sameLine && delta > 2 && delta < 32 then arrow delta else "^--"
