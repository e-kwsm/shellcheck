  ...
  where
    ...
    insertIn overwrite scope name val state =
      let (mapToCheck, inserter) =
       ...
       in if overwrite || not alreadyExists
...
analyzeControlFlow params t =
  ...
  where
    ...
    getFunctionTargets state =
      let declaredFuncs = S.unions $ M.elems $ mapStorage $ sFunctionTargets state
          getFunc d =
            case d of
