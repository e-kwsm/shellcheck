  ...
  where
    ...
    kludge params = bashism
      where
                ...
                if isBusyboxSh
                    then
                        ...
                        if isDash
                            ...
                            else warnMsg (getId arg) 3037 "echo flags are"
          where
        ...
        advancedExpansions =
            let re = mkRegex
             in [ (re $ "^![" ++ varChars ++ "]", 3053, "indirect expansion is")
        ...
        simpleExpansions =
            let re = mkRegex
             in [ (re $ "^[" ++ varChars ++ "*@]+:[^-=?+]", 3057, "string indexing is")
...
checkNegatedUnaryOps = ForShell [Bash] f
  where
    f token = case token of
        TC_Unary id SingleBracket "!" (TC_Unary _ _ op _)
            | op `elem` ["-a", "-o"] ->
                err id 2332 $ msg op
        _ -> return ()
