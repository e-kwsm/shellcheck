...
checkTr = CommandCheck (Basename "tr") (mapM_ f . arguments)
  where
    f w
        | isGlob w -- The user will go [ab] -> '[ab]' -> 'ab'. Fixme?
            =
    ...
    f word =
        case getLiteralString word of
            ...
            Just s -> do
                -- Eliminate false positives by only looking for dupes in SET2?
...
checkFindNameGlob = CommandCheck (Basename "find") (f . arguments)
  where
    ...
    g b acc a = do
        forM_ (getLiteralString a) $ \s ->
            when (acceptsGlob s && isGlob b) $
                warn (getId b) 2061 $
                    "Quote the parameter to " ++ s ++ " so the shell won't interpret it."
        acc b
...
checkExpr = CommandCheck (Basename "expr") f
  where
    f t = do
        when (all (`notElem` exceptions) (words $ arguments t)) $
            style
                (getId $ getCommandTokenOrThis t)
                2003
...
checkGrepRe = CommandCheck (Basename "grep") check
  where
    ...
    checkRE cmd (re : _) = do
        ...
        unless (any (`elem` flags) grepGlobFlags) $ do
            ...
            if isConfusedGlobRegex string
                then warn (getId re) 2063 "Grep uses regex, but this looks like a glob."
                else sequence_ $ do
...
checkTrapQuotes = CommandCheck (Exactly "trap") (f . arguments)
  where
    f (x : _) = checkTrap x
    f _ = return ()
                ...
                if
                        ...
                        | otherwise ->
                            warn (getId format) 2183 $
                                "This format string has "
                                    ++ show formatCount
                                    ++ " "
                                    ++ pluraliseIfMany "variable" formatCount
                                    ++ ", but is passed "
                                    ++ show argCount
                                    ++ pluraliseIfMany " argument" argCount
                                    ++ "."
...
checkUuoeCmd = CommandCheck (Exactly "echo") (f . arguments)
  where
...
checkAliasesUsesArgs = CommandCheck (Exactly "alias") (f . arguments)
  where
    ...
    f = mapM_ checkArg
    checkArg arg =
        let string = getLiteralStringDef "_" arg
         in when ('=' `elem` string && string `matches` re) $
                err
                    (getId arg)
                    2142
...
checkFindWithoutPath = CommandCheck (Basename "find") f
  where
    ...
    hasPath (first : rest) =
        let flag = getLiteralStringDef "___" first
         in not ("-" `isPrefixOf` flag) || isLeadingFlag flag && hasPath rest
    hasPath [] = False
...
checkTimeParameters = CommandCheck (Exactly "time") f
  where
    f (T_SimpleCommand _ _ (cmd : args : _)) =
        whenShell [Bash, Sh] $
            let s = concat $ oversimplify args
             in when ("-" `isPrefixOf` s && s /= "-p") $
...
checkTimedCommand = CommandCheck (Exactly "time") f
  where
...
checkLocalScope = CommandCheck (Exactly "local") $ \t ->
    whenShell [Bash, Dash, BusyboxSh] $ do
        -- Ksh allows it, Sh doesn't support local
        path <- getPathM t
...
checkMultipleDeclaring cmd = CommandCheck (Exactly cmd) (mapM_ check . arguments)
  where
    check t = sequence_ $ do
        ...
        return $
            err (getId $ getCommandTokenOrThis t) 2316 $
                "This applies "
                    ++ cmd
...
checkCatastrophicRm = CommandCheck (Basename "rm") $ \t ->
    when (isRecursive t) $
        mapM_ (mapM_ checkWord . braceExpand) $
            arguments t
  where
    ...
    checkWord' token = sequence_ $ do
        ...
        return . when (path `elem` importantPaths) $
            warn (getId token) 2115 $
                "Use \"${var:?}\" to ensure this never expands to " ++ path ++ " ."

    fixPath filename =
        let normalized = skipRepeating '/' . skipRepeating '*' $ filename
         in if normalized == "/" then normalized else stripTrailing '/' normalized

    getPotentialPath = getLiteralStringExt f
      where
        f (T_Glob _ str) = return str
        f (T_DollarBraced _ _ word) =
            let var = onlyLiteralString word
             in -- This shouldn't handle non-colon cases.
...
checkMaskedReturns str = CommandCheck (Exactly str) checkCmd
  where
    ...
    checkArgs (T_Assignment id _ _ _ word)
        | any hasReturn $ getWordParts word =
