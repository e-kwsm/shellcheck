...
defaultSpec pr =
  ...
  where
    spec = newAnalysisSpec (fromJust $ prRoot pr)
...
pScript s =
    let
     ...
     in
        runIdentity $ parseScript (mockedSystemInterface []) pSpec
...
determineShell fallbackShell t =
    fromMaybe Bash $
...
getModifiedVariables t =
    case t of
        ...
        TA_Unary _ op v@(TA_Variable _ name _)
            | "--" `isInfixOf` op || "++" `isInfixOf` op ->
  ...
  where
    ...
    f place t = case t of
        T_DollarBraced _ _ l ->
            let str = getBracedReference $ concat $ oversimplify l
             in do
...
getModifiedVariableCommand base@(T_SimpleCommand id cmdPrefix (T_NormalWord _ (T_Literal _ x : _) : rest)) =
                 ...
                 in fromMaybe fallback $ do
                        ...
                        case lookup "a" parsed of
                            ...
                            Nothing ->
                                return $
                                    catMaybes $
                                        map (getLiteral . snd . snd) $
                                            filter (null . fst) parsed
            "getopts" ->
                case rest of
  ...
  where
    ...
    letParamToLiteral token =
      ...
      where
        var = takeWhile isVariableChar $ dropWhile (`elem` "+-") $ concat $ oversimplify token
...
getReferencedVariables parents t =
    case t of
        T_DollarBraced id _ l ->
            let str = concat $ oversimplify l
             in (t, t, getBracedReference str)
                    : map
                        (\x -> (l, l, x))
