...
options =
    ...
    , Option
        "i"
        ["include"]
        (ReqArg (Flag "include") "CODE1,CODE2..")
        "Consider only given types of warnings"
    , Option
        "e"
        ["exclude"]
        (ReqArg (Flag "exclude") "CODE1,CODE2..")
        "Exclude types of warnings"
    , Option
        ""
        ["extended-analysis"]
        (ReqArg (Flag "extended-analysis") "bool")
    ...
    , Option
        ""
        ["list-optional"]
        (NoArg $ Flag "list-optional" "true")
        "List checks disabled by default"
    , Option
        ""
        ["norc"]
        (NoArg $ Flag "norc" "true")
    ...
    , Option
        "x"
        ["external-sources"]
        (NoArg $ Flag "externals" "true")
        "Allow 'source' outside of FILES"
    , Option
        ""
        ["help"]
        (NoArg $ Flag "help" "true")
...
ioInterface options files = do
    ...
    return
        (newSystemInterface :: SystemInterface IO)
  ...
  where
                ...
                if path == "/"
                    then return result
                        ...
                        when (isNothing result) $
                            hPutStrLn stderr $
                                "Warning: unable to read --rcfile " ++ file
    ...
    getEditorConfig filename = do
      ...
      where
        ...
        readGlobalEditorConfig = do
            path <-
                (getXdgDirectory XdgConfig "editorconfig.ini")
    ...
    findSourceFile inputs sourcePathFlag currentScript rcSuggestsExternal sourcePathAnnotation original =
        if isAbsolute original
            ...
            else find original original
      where
        find filename deflt = do
            sources <-
                findM ((allowable rcSuggestsExternal inputs) `andM` doesFileExist) $
